...
import { isFiniteNumber } from "@/lib/scan/geometry";
...
type Props = {
  walls: Wall[];
  highlightedWall?: WallId | null;
  /** Optional per-wall overrides; defaults to the openings on each wall. */
  doors?: Partial<Record<WallId, Opening[]>>;
  windows?: Partial<Record<WallId, Opening[]>>;
  className?: string;
};
...
export function RoomDiagramSVG({
  walls,
  ...
  return (
    <svg
      ...
      className={className}
      role="img"
      aria-label={describeRoom(walls, widthM, depthM)}
    >
      <rect
        x={geometry.x0}
        y={geometry.y0}
      ...
      {(["N", "E", "S", "W"] as const).map((id) => (
        <WallShape
          key={id}
          id={id}
          wall={byId.get(id)}
          geometry={geometry}
          highlighted={highlightedWall === id}
          doors={doors?.[id] ?? byId.get(id)?.doors ?? []}
          windows={windows?.[id] ?? byId.get(id)?.windows ?? []}
        />
      ))}
    </svg>
  );
...
function WallShape({
  ...
  return (
    <g>
      {solidSpans(doors, drawnLengthM).map(([from, to], index) => {
      ...
      <WallLabel
        ...
        text={
          isFiniteNumber(measured) && measured > 0
            ? `${WALL_LABELS[id]} ${measured.toFixed(2)} m`
            : WALL_LABELS[id]
        }
      />
    </g>
...
function DoorMark({
  ...
  return (
    <path
      d={`M ${hinge.x} ${hinge.y} L ${line.pointAt(to).x} ${line.pointAt(to).y} M ${
        line.pointAt(to).x
      } ${line.pointAt(to).y} A ${radius} ${radius} 0 0 1 ${swingEnd.x} ${swingEnd.y}`}
      fill="none"
      stroke={LABEL_COLOR}
      strokeWidth={1.25}
      strokeDasharray="3 2"
...
function WallLabel({
  ...
  return (
    <text
      ...
      fill={highlighted ? HIGHLIGHT_COLOR : LABEL_COLOR}
    >
      {text}
    </text>
  );
...
type WallLine = {
  ...
  scale: number;
  /**
   * Point `metres` along the wall, pushed `outward` pixels away from the room
   * centre (negative pushes inward).
   */
  pointAt: (metres: number, outward?: number) => { x: number; y: number };
};

function wallLine(id: WallId, geometry: Geometry): WallLine {
...
function clampSpan(
  ...
  if (!isFiniteNumber(opening.offsetFromLeft) || !isFiniteNumber(opening.width)) {
    return null;
  }
 ...
 */
function solidSpans(doors: Opening[], wallLengthM: number): Array<[number, number]> {
  const gaps = doors
  ...
  if (cursor < wallLengthM) spans.push([cursor, wallLengthM]);

  return spans;
}

function describeRoom(walls: Wall[], widthM: number, depthM: number): string {
  const openings = walls.reduce(
